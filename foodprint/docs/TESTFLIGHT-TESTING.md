...
- Choose **Log food / drink → Food → Type a name**. Try `bread`, answer its variant question, and tap **Review ingredients**. Repeat with `gluten-free bread` and `lactose-free milk`; standard wheat or lactose assumptions should not survive the relevant exclusion. Check the actual recipe before confirming anything.
...
- On a real iPhone/iPad, choose **Scan a label → Take photo**. Keep the Ingredients heading visible and crop out other panels. Check camera permission denial, cancellation and **Choose photo**, then a clear label and a difficult/glare-filled one.
- Compare every proposed ingredient with the packet. Nested ingredients should remain understandable; nutrition/storage text should not become ingredients. `Contains` declarations and `May contain` warnings must remain separate from confirmed consumption.
- A scan without a clear Ingredients heading should request manual selection. Select/type only that section, enable **I selected only the ingredients**, and parse again. Correct mistakes before **I checked these against the label** and save. Reopen the entry: its saved label text and notes should remain.
- **Find a product** accepts a product name or a typed barcode number; camera barcode scanning is not implemented. Check the exact brand/formulation and its current packet. Open Food Facts is a community catalog with incomplete or outdated records, not a complete product database. Missing ingredients should prompt manual entry/scanning; outages and repeated searches should give a recoverable message.
