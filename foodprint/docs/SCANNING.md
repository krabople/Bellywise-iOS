...
The outer repository's `.github/workflows/ci.yml` runs TypeScript/tests and a separate unsigned iOS Simulator compile on `macos-26` with Node 24. `scripts/ci/build-ios.sh` performs Expo prebuild, CocoaPods installation and Xcode Release compilation, writing build logs and a simulator app artifact. It checks that both the Vision module and SQLCipher are linked. SDK 57 requires Xcode 26.4+ and iOS 16.4+; the macOS runner currently supplies a suitable Xcode. [Expo SDK requirements](https://docs.expo.dev/versions/latest/), [GitHub macOS runner software](https://github.com/actions/runner-images/blob/main/images/macos/macos-26-Readme.md).
