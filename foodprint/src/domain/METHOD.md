...
The bundled recognition catalogue combines a small curated set with the Open Food Facts ingredient taxonomy pinned to source revision `b0c557fe6b54edf1b2e1ff4d1298ead345bd508a`. Its generated names and synonyms are distributed under the Open Database License; source and generation metadata live with the generated catalogue. Matching uses complete aliases and phrases, normalises known synonyms to one canonical entry (for example, niacin and vitamin B3), and excludes negated/free-from phrases and advisory text. Preparation and appearance variants such as fresh, dried, frozen, chopped or powdered forms resolve through taxonomy parents to a shared curated food family when the relationship is unambiguous. Materially different derivatives and species, such as garlic oil or wild garlic, stay distinct. It does not pull isolated words from unrelated packaging copy. A camera label can therefore be recognised without an explicit Ingredients heading when a line has strong catalogue coverage, while uncertain text remains for review instead of becoming an exposure.

Barcode lookup uses the product service's structured ingredient nodes. The product name and ingredient data come from that record; Bellywise does not infer a scanned product's ingredients from its name. The raw published label remains visible for audit. Open Food Facts is community-maintained, so records may still be incomplete or inaccurate and the user must review them.

An exposure day is confirmed if any logged meal explicitly confirms that ingredient. The comparison reports how many exposure days depend entirely on inference. Confirmed-only mode excludes an inferred exposure day; it never relabels that day as an unexposed control. Missing ingredients in a recipe can still misclassify absence, so reviewing labels and actual recipes is essential. Unknown food names are preserved with inferred confidence instead of receiving fabricated ingredients. If a manually entered ingredient has no exact catalogue match, the app says so, offers close matches, and requires an explicit choice before storing the original term as a persistent personal ingredient.
