...
export function parseDiary(raw: string): SavedDiary {
  ...
  if (!d.checkIns.every(c => record(c) && calendarDate(c.date) && c.date <= localDateKey(new Date()) && typeof c.complete === 'boolean' && (c.noSymptomsConfirmed === undefined || typeof c.noSymptomsConfirmed === 'boolean') && (c.trackedSymptomIds === undefined || (list(c.trackedSymptomIds) && c.trackedSymptomIds.every(identifier) && new Set(c.trackedSymptomIds).size === c.trackedSymptomIds.length)) && level(c.stress) && (c.sleepHours === undefined || (Number.isFinite(c.sleepHours) && c.sleepHours >= 0 && c.sleepHours <= 24)) && optionalText(c.notes))) throw new Error('The backup contains an invalid check-in.');
  ...
  if (d.notificationPreferences !== undefined) {
    ...
    const validTime = (hour: unknown, minute: unknown) => Number.isInteger(hour) && Number(hour) >= 0 && Number(hour) <= 23 && Number.isInteger(minute) && Number(minute) >= 0 && Number(minute) <= 59;
    const validCommon = record(n) && typeof n.patternAlertsEnabled === 'boolean' && list(n.notifiedPatternKeys) && n.notifiedPatternKeys.every(identifier) && new Set(n.notifiedPatternKeys).size === n.notifiedPatternKeys.length;
    const legacy = validCommon && typeof n.dailyReminderEnabled === 'boolean' && validTime(n.dailyReminderHour, n.dailyReminderMinute) && (n.dailyReminderId === undefined || identifier(n.dailyReminderId));
    const modern = validCommon && typeof n.foodRemindersEnabled === 'boolean' && list(n.foodReminderTimes) && n.foodReminderTimes.length >= 1 && n.foodReminderTimes.length <= 3 && n.foodReminderTimes.every((time: unknown) => record(time) && identifier(time.key) && validTime(time.hour, time.minute) && (time.notificationId === undefined || identifier(time.notificationId))) && new Set(n.foodReminderTimes.map((time: any) => time.key)).size === n.foodReminderTimes.length && typeof n.dayReviewReminderEnabled === 'boolean' && validTime(n.dayReviewReminderHour, n.dayReviewReminderMinute) && (n.dayReviewReminderId === undefined || identifier(n.dayReviewReminderId));
    if (!legacy && !modern) throw new Error('The backup contains invalid notification preferences.');
  }
